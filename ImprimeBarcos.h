...
{
	// Recibe: 4 matrices con basura de tamaño: 'tamTab'*'tamTabSe'
	// Sale: Se modifican sus componentes y se guarda en todos ellos 'vacio'
...
{
	// Recibe: Se recibe una matriz de tamaño: 'tamTab'*'tamTab' con datos.
	// Sale: Se imprime por pantalla su contenido.
...
*/
	...
	if( longCoor == 2   &&   coor[0] >= 48 && coor[0] <= 48+tamTab-1   &&   coor[1] >= 97 && coor[1] <= 97+tamTab-1 )
	{
		...
		*col = coor[1]-49-48;
		...
		// Si en la coordenada elegida ya hay un barco, se imprime un error y correcto se cambia a falso.
...
// Función para imprimir las posibles posiciones del barco.
void impMovPosBarco( char tablero[tamTab][tamTab][2], int fil, int col, int longBar )
{
	/*
		...
		Sale:	Se guarda en los componentes correspondientes todas las posiciones que puede tomar el final del barco, además de imprimir la matriz por pantalla.
	*/
	...
	for( i=0; i < 4; i++ )
	{
		...
		huecosLibres = 1; // Se cambia a 1 el contador de huecos libres.

	};

};
// Función que valida si la posición final es o no 'fin'
int hayFinMarcado( char tablero[tamTab][tamTab][2], int filFin, int colFin )
{
	/*
	 	Recibe:
	 					recibe una matriz de tamaño: 'tamTab'*'tamTab'
	 					Y dos enteros no mayores que 'tamTab'

		Sale:	Se devolverá un 1/true o un 0/false según corresponda.
	 */
	int esFin = 1;
...
// Función que devuelve la coordenada menor (mas cerca del cero)
int devCoorMenor( int coorUno, int coorDos )
{
	/*
		Recibe:	recibe dos enteros no mayores que 'tamTab'
		Sale:	devuelve el numero más pequeño.
	*/
...
// Función que imprime el resto del barco según elija el jugador/maquina.
void impFinBarco( char tablero[tamTab][tamTab][2], int fil, int col, int longBar, char simbBarco, int tipoJugador )
{
	/*
		...
		Sale:	Se guarda en todos los componentes que ocupa ese barco el simbBarco recibido.
	*/
	char coor[4];
	int coorMenor;
	int filFin, colFin;
	...
	else // En caso contrario, la maquina genera sus coordenadas:
	{
		...
		} while( maqGuaCoor != 1 );	// Mientras no se pueda colocar el barco en una de las posibles posiciones se repete el proceso
...
// Función que borra las posibles soluciones.
void borrMovPosBarco( char tablero[tamTab][tamTab][2] )
{
	// Recibe: una matriz de tamaño: 'tamTab'*'tamTab'
	// Sale: Se sustituyen todos los caracteres 'fin' por 'vacio'
...
void impBarco( char tabJug[tamTab][tamTab][2], char tabMaq[tamTab][tamTab][2], int longBar, int simAUsar, int tipoJugador )
{
	/*
		Recibe:
					Recibe dos matrices de tamaño: 'tamTab'*'tamTab'
		...
		Sale:	Se almacena en los componentes correspondientes el barco escogido, además de imprimir por pantalla, solo con el usuario, su tablero.
	*/
	...
	else // En caso contrario, la maquina genera sus coordenadas:
	{
		do
		{
			// Se generan las coordenadas de forma aleatoria
			coor[0] = (rand()%tamTab)+48;		// Un numero del 0 al tamTab al cual se le suman 48 para obtener el numero ASCII correspondiente.
			coor[1] = (rand()%tamTab)+48+49;	// Una numero del 0 a tamTab, que posteriormente se le suman 48 para tener el numero en ASCII y +49 para la letra.
			coor[2] = '\0';
